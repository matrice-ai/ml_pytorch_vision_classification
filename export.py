...
class iOSModel(torch.nn.Module):
    ...
    def __init__(self, model, im):
        ...
        self.normalize = self._calculate_normalize_factor(w,h)
# added by me 
    def _calculate_normalize_factor(self,w,h):
         #"calcualte normaliation factor for bounding box coordinates"    
        if w == h:
            self.normalize = 1. / w
        else:
...
def export_formats():
    # Return a dataframe of YOLOv5 export formats
    formats = [
    ...
    return pd.DataFrame(formats, columns=['Format', 'Argument', 'Suffix', 'CPU', 'GPU'])
...
def export_torchscript(model, im, file, optimize, prefix=colorstr('TorchScript:')):
    ...
    #d = {'shape': im.shape, 'stride': int(max(model.stride)), 'names': None}
...
def _create_extra_files(shape, stride):# create extra files with model configuration detials ""
    d = {'shape': shape, 'stride': int(max(stride)), 'names': None}
    return{'config.txt': json.dumps(d)}

def _save_optimized_model(ts,file,extra_files):#save optimized model for mobile "
    optimize_for_mobile(ts)._save_for_lite_interpreter(str(file),_extra_files=extra_files)
 
def try_exports(inner_func):
    #"yolov5 export decorator to handle sucess and failiure logging"
    inner_args = get_default_args(inner_func)
...
def export_saved_model(model,
    ...
    from export_utils.models.tf import TFModel
...
def export_tflite(keras_model, im, file, int8, data, nms, agnostic_nms, prefix=colorstr('TensorFlow Lite:')):
    ...
    import tensorflow as tf
    try:
     actionTracker.update_status('MDL_EXPT_ACK', 'OK', 'TFLITE Model Export has been acknowledged')
     #LOGGER.info(f'\n{prefix} starting export with tensorflow {tf.__version__}...')
     batch_size, ch, *imgsz = list(im.shape)  # BCHW
    ...
    except Exception as e :#handle the excepion and return none along with the error msg 
       error_message = f'Model not exported:{str(e)}'
...
def pipeline_coreml(model, im, file, names, y, prefix=colorstr('CoreML Pipeline:')):
    ...
    try:
        ...
        #attempting to prediction on macos using coreml model 
        img = Image.new('RGB', (w, h))  # img(192 width, 320 height)
        # img = torch.zeros((*opt.img_size, 3)).numpy()  # img size(320,192,3) iDetection
        out = model.predict({'image': img})
        out0_shape, out1_shape = out[out0.name].shape, out[out1.name].shape
    except Exception as e:  # linux and windows can not run model.predict(), get sizes from pytorch output y
        s = tuple(y[0].shape)
    ...
    pipeline.spec.description.metadata.userDefined.update({'classes': ','.join(names.values()),'iou_threshold': str(nms.iouThreshold),'confidence_threshold': str(nms.confidenceThreshold)})
...
try:
    ...
    metadata = {'stride': int(max(model.stride)), 'names': 'string'}  # model metadata
